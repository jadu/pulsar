...
class HelperOptionsModifierExtension extends \Twig_Extension
{
    ...
    public function modifyOptions($options)
    {
        ...
        if is_array($options['error']) and (array_key_exists('error', $options) and count($options['error']) > 0) {
        ...
        // Check if errors are present in options
        if is_array($options['help']) and (array_key_exists('help', $options) and count($options['help']) > 0) {
