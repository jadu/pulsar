...
describe('DropZone', () => {
    ...
    describe('setup()', () => {
        beforeEach(() => {
            dropZone = new DropZone({ node: document.getElementById('dropzone') });
        });

        it('should create an empty event pool', () => {
            dropZone.eventPool = ['foo'];
            dropZone.setup();
            expect(dropZone.eventPool.length).to.equal(0);
        });

        it('should create an empty file array', () => {
            dropZone.files = ['foo'];
            dropZone.setup();
            expect(dropZone.files.length).to.equal(0);
        });

        it('should create an empty event tracker object', () => {
            dropZone.eventTracker = 'foo';
            dropZone.setup();
            expect(dropZone.eventTracker).to.deep.equal({ node: {}, window: {} });
        });

        it('should create the size count', () => {
            dropZone.size = 999;
            dropZone.setup();
            expect(dropZone.size).to.equal(0);
        });

        it('should create window active state', () => {
            dropZone.windowActive = 'foo';
            dropZone.setup();
            expect(dropZone.windowActive).to.be.false;
        });

        it('should create/reset DropZone active state', () => {
            dropZone.dropZoneActive = 'foo';
            dropZone.setup();
    ...
    describe('registerEvents()', () => {
        ...
        //
        // before(() => {
        //     handleDropStub = stub(DropZone.prototype, 'handleDrop')
        //     dropeZone.handleDropWithContext = DropZone.prototype.handleDrop
        // });
        //
        // after(() => {
        //     dropZone.handleDropWithContext.restore();
        //     dropZone.handleEnterWithContext.restore();
        // });
        //
        ...
        //
        //     expect(handleDropStub).to.be.have.been.calledOnce;
        // });
    ...
    describe('resetEventTracker()', () => {
        beforeEach(() => {
            dropZone = new DropZone({ node: document.body });
        });

        it('should reset all of the event tracker counts to 0', () => {
            dropZone.eventTracker = { node: { click: 10, hover: 20 }, window: { click: 99, hover: 200 } };
            dropZone.resetEventTracker();
            expect(dropZone.eventTracker).to.deep.equal({
                node: { click: 0, hover: 0 },
                window: { click: 0, hover: 0 }
            });
        });
    });
});
